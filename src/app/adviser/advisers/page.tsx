...
import { getAdvisers, type Adviser } from "@/lib/actions/advisers";

export default function AdviserDirectoryPage() {
    ...
    <div style={{ fontFamily: "var(--font-body)" }}>
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          View all registered advisers and their verification status on the platform.
        </p>
      </div>
            ...
            {advisers.length === 0 ? (
              ...
              advisers.map((adviser) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-xs text-slate-500">
                    {adviser.organization_name || <span className="italic text-slate-400">None</span>}
                  </td>
                  <td className="px-4 py-3 text-xs text-slate-500">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full border px-2 py-0.5 text-xs font-medium uppercase
                      ${adviser.status === 'pending' ? 'border-warning/30 bg-warning/10 text-warning' : 
                        ...
                        adviser.status === 'rejected' ? 'border-destructive/30 bg-destructive/10 text-destructive' :
      ...
      <div className="mt-4 flex items-center justify-between">
        <p className="text-sm text-slate-500">
          ...
          {Math.min(page * limit, totalCount)} of {totalCount} advisers
        </p>
