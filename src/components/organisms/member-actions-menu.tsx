          ...
          {tags.map((tag) => (
            ...
            >
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-700 rounded-full font-bold leading-none w-4 h-4 flex items-center justify-center cursor-pointer"
              >
                &times;
              </button>
            </div>
  ...
  return (
    <div className="relative flex justify-center" ref={menuRef}>
      <button
        ...
        className="text-slate-400 transition-colors hover:text-foreground hover:bg-slate-100 p-2 rounded-full w-8 h-8 flex items-center justify-center cursor-pointer"
      >
        ⋯
      </button>
      ...
      {isOpen && (
        <div
          className={`absolute right-0 ${openUpwards ? "bottom-full mb-1" : "top-full mt-1"} w-44 bg-white border border-border rounded-xl shadow-lg z-10 py-1 overflow-hidden`}
        >
          <button
            onClick={() => {
      ...
      <RenameModal
        key={`rename-${isRenameOpen ? "open" : "closed"}`}
        isOpen={isRenameOpen}
        onClose={() => setIsRenameOpen(false)}
        member={member}
        onSave={(name: string) =>
          onRename(member.membership_id, member.user_id, name)
        }
      />
      <RoleModal
        key={`role-${isRoleOpen ? "open" : "closed"}`}
        isOpen={isRoleOpen}
        onClose={() => setIsRoleOpen(false)}
        member={member}
        onSave={(role: string) => onChangeRole(member.membership_id, role)}
      />
      <TagsModal
        key={`tags-${isTagsOpen ? "open" : "closed"}`}
        isOpen={isTagsOpen}
        onClose={() => setIsTagsOpen(false)}
        member={member}
        onSave={(tags: string[]) => onUpdateTags(member.membership_id, tags)}
      />
      ...
      <FeedbackModal
        ...
        tone="error"
        onConfirm={() => {
          onRemove(member.membership_id, member.user_id);
