    ...
    <div className="flex-1 w-full bg-white border border-slate-200 rounded-xl overflow-visible shadow-sm animate-in fade-in duration-300">
        ...
        <div className="flex items-center gap-4">
          <div className="flex gap-2">
            <button
              ...
              className="p-1.5 sm:p-2 bg-white hover:bg-slate-50 rounded-md border border-slate-200 transition-colors shadow-sm"
            >
              <svg
      ...
      <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
          <div
            key={day}
            className="py-2.5 text-center text-[10px] font-bold text-slate-500 uppercase tracking-wider"
          >
            {day}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 rounded-b-xl overflow-hidden">
        {blanks.map((blank) => (
          <div
            key={`blank-${blank}`}
            className="border-b border-r border-slate-100 bg-slate-50/50 p-2 min-h-25 sm:min-h-30"
          />
        ))}
