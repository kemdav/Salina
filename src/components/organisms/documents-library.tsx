...
export function DocumentsLibrary({ 
  ...
  useEffect(() => {
    if (canManage) {
      getMembers().then(m => {
        setMembersList(m.map(member => ({ id: member.userId, name: member.name })));
      }).catch(console.error);
    }
  }, [canManage]);
  ...
  const filteredDocuments = useMemo(() => {
    ...
    if (searchQuery) result = result.filter(d => d.title.toLowerCase().includes(searchQuery.toLowerCase()) || d.file_name.toLowerCase().includes(searchQuery.toLowerCase()));
    result.sort((a, b) => {
  ...
  async function performDelete(id: string, type: "folder" | "document") {
    try {
      ...
      setFeedbackConfig({ isOpen: true, title: "Delete Failed", message: error instanceof Error ? error.message : "Failed to delete.", tone: "error", showCancel: false });
    }
  }
  ...
  async function handleUpload(formData: FormData) {
    try {
      ...
      if (visibility === "roles") formData.append("allowed_roles", selectedRoles.join(","));
      if (visibility === "members") formData.append("allowed_members", selectedMembers.join(","));

      await uploadDocument(formData);
      setShowUploadModal(false);
      resetVisibilityState();
    } catch (error: unknown) {
      setFeedbackConfig({ isOpen: true, title: "Upload Failed", message: error instanceof Error ? error.message : "Failed to upload document", tone: "error", showCancel: false });
    } finally {
  ...
  async function handleCreateFolder(e: React.FormEvent) {
    ...
    try {
      await createFolder(newFolderName.trim(), currentFolderId, visibility, password, selectedRoles, selectedMembers);
      setShowNewFolderModal(false);
      setNewFolderName("");
      resetVisibilityState();
    } catch (error: unknown) {
      setFeedbackConfig({ isOpen: true, title: "Create Folder Failed", message: error instanceof Error ? error.message : "Failed to create folder", tone: "error", showCancel: false });
    }
  }
  ...
  async function handleRename(e: React.FormEvent) {
    ...
    try {
      if (renameTarget.type === "folder") await renameFolder(renameTarget.id, renameValue.trim());
      else await renameDocument(renameTarget.id, renameValue.trim());
      setRenameTarget(null);
    } catch (error: unknown) {
      setFeedbackConfig({ isOpen: true, title: "Rename Failed", message: error instanceof Error ? error.message : "Failed to rename", tone: "error", showCancel: false });
    }
  }
  ...
  async function handleFolderClick(folderId: string) {
    ...
    if (access === 'granted') {
      ...
      setFeedbackConfig({ isOpen: true, title: "Access Denied", message: "You do not have permission to view this folder.", tone: "error", showCancel: false });
    }
  }
  ...
  const renderVisibilityOptions = () => (
    <div className="space-y-4 border-t border-slate-100 pt-4 mt-4">
      <div>
        ...
        <select id="visibility-select" value={visibility} onChange={(e) => setVisibility(e.target.value as "public" | "roles" | "members" | "password_protected")} className="mt-1 flex h-10 w-full rounded-xl border border-border bg-white px-3 py-2 text-sm text-slate-900 ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2">
      ...
      {visibility === "password_protected" && (
        <div>
          ...
          <Input id="password-input" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter a secure password" />
        </div>
      )}
      ...
      {visibility === "members" && (
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-2 max-h-40 overflow-y-auto border border-slate-200 rounded-xl p-2 bg-slate-50">
            {membersList.length === 0 && <p className="text-xs text-slate-500 p-2">Loading members...</p>}
            {membersList.map(member => (
              <label key={member.id} className="flex items-center gap-2 cursor-pointer px-2 py-1 hover:bg-slate-100 rounded">
                <input type="checkbox" checked={selectedMembers.includes(member.id)} onChange={() => toggleMember(member.id)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center text-sm text-slate-500 overflow-x-auto whitespace-nowrap pb-1">
            <button onClick={() => navigateTo(null)} className="hover:text-blue-600 hover:underline font-medium" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, null)}>
              Root
            </button>
            {breadcrumbs.map((crumb) => (
              <span key={crumb.id} className="flex items-center">
                ...
                <button onClick={() => handleFolderClick(crumb.id)} className="hover:text-blue-600 hover:underline font-medium" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, crumb.id)}>
                  {crumb.name}
                </button>
              </span>
            ))}
        ...
        {canManage && (
          <div className="flex gap-2">
            <Button variant="secondary" onClick={() => { resetVisibilityState(); setShowNewFolderModal(true); }}>New Folder</Button>
            <Button onClick={() => { resetVisibilityState(); setShowUploadModal(true); }}>Upload</Button>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
        <div className="w-full sm:w-64">
          <Input placeholder="Search files..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-white" />
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value as "name" | "date" | "size")} className="h-10 rounded-xl border border-border bg-white px-3 py-2 text-sm text-slate-900 outline-none">
            <option value="name">Sort by Name</option>
            <option value="date">Sort by Date</option>
            <option value="size">Sort by Size</option>
          </select>
          <div className="flex bg-white border border-border rounded-xl overflow-hidden p-0.5">
            <button onClick={() => setViewMode("grid")} className={`p-2 rounded-lg ${viewMode === "grid" ? "bg-slate-100 text-slate-900" : "text-slate-400 hover:text-slate-600"}`}>
              <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
            </button>
            <button onClick={() => setViewMode("list")} className={`p-2 rounded-lg ${viewMode === "list" ? "bg-slate-100 text-slate-900" : "text-slate-400 hover:text-slate-600"}`}>
        ...
        {filteredFolders.length === 0 && filteredDocuments.length === 0 ? (
          ...
          <>
            {filteredFolders.map((folder) => (
              <div key={folder.id} className={`group relative flex ${viewMode === "grid" ? "flex-col p-5" : "flex-row items-center p-3"} rounded-2xl border border-border bg-white shadow-sm transition-all hover:shadow-md cursor-pointer hover:border-blue-200`} onClick={() => handleFolderClick(folder.id)} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, folder.id)}>
            ...
            {filteredDocuments.map((doc) => (
              <div key={doc.id} draggable={canManage} onDragStart={(e) => handleDragStart(e, doc.id, "document")} className={`group relative flex ${viewMode === "grid" ? "flex-col p-5" : "flex-row items-center p-3"} rounded-2xl border border-border bg-white shadow-sm transition-all hover:shadow-md cursor-grab active:cursor-grabbing`}>
                {viewMode === "grid" ? (
                  <>
                    <div className="mb-3 flex items-start justify-between">
                      <div className="flex gap-2">
                        ...
                        {doc.visibility !== "public" && (
                          <Badge variant="secondary" className="text-slate-500 border-slate-200 bg-slate-50">
                            ...
                            {doc.visibility === "password_protected" ? "Locked" : "Restricted"}
                          </Badge>
                        )}
                      </div>
                      {canManage && (
                  ...
                  <div className="w-full flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4 flex-1">
                      <div className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center shrink-0 border border-slate-100 relative">
                        {doc.visibility === "password_protected" && <div className="absolute -top-1 -right-1 w-3 h-3 bg-slate-200 rounded-full border border-white flex items-center justify-center"><svg className="w-2 h-2 text-slate-600" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" /></svg></div>}
      ...
      {unlockTarget && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" aria-hidden onClick={() => setUnlockTarget(null)} />
          <div role="dialog" aria-modal="true" className="relative flex w-full max-w-md flex-col rounded-2xl border border-border bg-white shadow-2xl">
            <form onSubmit={handleUnlockSubmit} className="flex flex-col">
              <div className="p-6 pb-0">
                ...
                <p className="text-sm text-slate-500 mb-4">This folder is password protected. Please enter the password to view its contents.</p>
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="unlock-password">Password</Label>
                    <Input id="unlock-password" type="password" required autoFocus value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} />
                  </div>
                </div>
              </div>
              <div className="mt-8 flex justify-end gap-3 rounded-b-2xl border-t border-slate-100 bg-slate-50 p-4">
