...
import { createRole, updateRole, deleteRole, type OrganizationRole } from "@/lib/actions/roles";

export function RolesManager({ initialRoles }: { initialRoles: OrganizationRole[] }) {
  const [isPending, setIsPending] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [optimisticRoles, setOptimisticRoles] = useOptimistic(
    initialRoles,
    (state, action: { type: "add" | "update" | "delete"; role: Partial<OrganizationRole> & { id: string } }) => {
      switch (action.type) {
        case "add":
          return [...state, action.role as OrganizationRole];
        case "update":
          return state.map((r) => (r.id === action.role.id ? { ...r, ...action.role } : r));
        case "delete":
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRole) {
        ...
        setOptimisticRoles({ type: "update", role: { id: editingRole.id, ...payload } });
    ...
    <div style={{ fontFamily: "var(--font-body)" }}>
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <button 
          ...
          className="h-9 px-4 text-sm bg-primary text-white rounded-md hover:bg-primary/90 transition-colors pointer-events-auto" 
        >
          + Create Role
        </button>
      </div>
        ...
        <div className="mb-6 rounded-2xl border border-border bg-white shadow-sm p-6 animate-in fade-in slide-in-from-top-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                {AVAILABLE_PERMISSIONS.map((perm) => (
                   <label key={perm} className="flex flex-row items-center gap-2 border rounded-md p-3 cursor-pointer hover:bg-slate-50 transition-colors">
                     <input 
                       type="checkbox" 
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-border">
              <Button type="button" variant="ghost" onClick={handleCloseForm} disabled={isPending}>
                Cancel
              </Button>
                ...
                optimisticRoles.map((role) => (
                  ...
                  >
                    <td className="px-4 py-3 text-sm font-medium text-foreground">
                      {role.name}
                      {role.description && <p className="text-xs text-slate-500 font-normal mt-0.5">{role.description}</p>}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          title={role.member_count! > 0 ? "Cannot delete roles with active members" : "Delete role"}
                        >
                          Delete
                        </button>
                      </div>
