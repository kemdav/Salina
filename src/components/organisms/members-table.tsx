...
import { Member, updateMemberStatus, updateMemberDues, removeMember, updateMemberRole, updateMemberTags, updateMemberName } from '@/lib/actions/members';
...
export function MembersTable({ initialMembers }: { initialMembers: Member[] }) {
    ...
    const [optimisticMembers, setOptimisticMembers] = useOptimistic(
        ...
        (state, action: Action) => {
            switch(action.type) {
                ...
                case 'role':
                case 'name':
                    return state.map(m => m.membership_id === action.id ? { ...m, [action.type]: action.payload } : m);
                case 'tags':
                    return state.map(m => m.membership_id === action.id ? { ...m, tags: action.payload } : m);
                case 'remove':
    ...
    const filtered = optimisticMembers.filter((m) => {
        ...
        const matchesTag    = filterTag === 'All Tags' ||
            ...
            (filterTag !== 'Paid' && filterTag !== 'Unpaid' && m.tags.includes(filterTag));
        return matchesSearch && matchesStatus && matchesTag;
    });

    const handleStatusChange = (id: string, currentStatus: string) => {
        const statuses = ['Active', 'Probation', 'Alumni', 'Suspended'];
        const nextStatus = statuses[(statuses.indexOf(currentStatus) + 1) % statuses.length] as import('@/lib/actions/members').Status;
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-border bg-slate-50/50">
                            {['Member', 'Email', 'Role', 'Status', 'Dues', 'Tags', 'Joined', 'Actions'].map((col, i, arr) => (
                                <th
                                    key={col}
                    ...
                    <tbody>
                        {filtered.length === 0 ? (
                            ...
                            filtered.map((member) => {
                                const initials = (member.name || member.email || 'UU').slice(0, 2).toUpperCase();
                                ...
                                >
                                    <td className="px-4 py-3 text-sm">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="font-medium text-foreground">{member.name || 'Unnamed Member'}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-4 py-3">
                                        <Badge className={`capitalize ${ROLE_CLASS[(member.role || '').toLowerCase()] || ROLE_CLASS.member}`}>
                                            {member.role}
                                        </Badge>
                                    </td>
                                    <td className="px-4 py-3">
                                        <button onClick={() => handleStatusChange(member.membership_id, member.status)} className="focus:outline-none">
                                            <Badge className={STATUS_CLASS[member.status] || STATUS_CLASS.Active}>
                                                {member.status}
                                            </Badge>
                                        </button>
                                    </td>
                                    <td className="px-4 py-3">
                                        <button onClick={() => handleDuesChange(member.membership_id, member.dues)} className="focus:outline-none">
                                            <Badge className={DUES_CLASS[member.dues] || DUES_CLASS.Unpaid}>
                                                {member.dues}
                                            </Badge>
                                        </button>
                                    </td>
