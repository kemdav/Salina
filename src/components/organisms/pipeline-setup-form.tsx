    ...
    <div className="flex flex-col w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8 lg:mb-10 w-full">
        ...
        <p className="text-sm text-[var(--muted)] mt-3">
          Structure the journey. Define how your candidates go through the
          process.
        </p>
      </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="border-b border-border pb-2">
              ...
              <p className="text-xs text-[var(--muted)] mt-1">
                Drag to reorder the flow. You can edit this as you like.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              {stages.map((stage, index) => (
                <div
                  ...
                  className={`flex items-center gap-3 bg-white border border-[#E5E7EB] rounded-(--radius) p-2 transition-all ${draggingIdx === index ? "opacity-50 scale-[0.98] shadow-inner bg-slate-50" : "shadow-sm hover:border-slate-300"}`}
                >
            ...
            <button
              ...
              className="mt-2 flex items-center justify-center gap-2 w-full py-2.5 rounded-(--radius) border border-dashed border-[#E5E7EB] text-sm font-medium text-[var(--muted)] hover:bg-slate-50 hover:text-foreground hover:border-slate-300 transition-colors"
            >
              <svg
                width="14"
                height="14"
          ...
          <div className="w-full h-full bg-[#F9FAFB] border border-[#E5E7EB] rounded-2xl p-5 sm:p-6 flex flex-col shadow-inner overflow-hidden">
            ...
            <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200">
              ...
              <div className="p-5 flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-4">
                  <div
                    className={`flex flex-col gap-1.5 transition-all duration-300 ${reqResume ? "opacity-100 h-auto" : "opacity-0 h-0 overflow-hidden hidden"}`}
                  >
                    <label className="text-xs font-semibold text-slate-600">
                  ...
                  <div
                    className={`flex flex-col gap-1.5 transition-all duration-300 ${reqCoverLetter ? "opacity-100 h-auto" : "opacity-0 h-0 overflow-hidden hidden"}`}
                  >
                    <label className="text-xs font-semibold text-slate-600">
                      Cover Letter
                    </label>
                  ...
                  <div
                    className={`flex flex-col gap-1.5 transition-all duration-300 ${reqPortfolio ? "opacity-100 h-auto" : "opacity-0 h-0 overflow-hidden hidden"}`}
                  >
                    <label className="text-xs font-semibold text-slate-600">
                      Portfolio Website
                    </label>
                  ...
                  <div
                    className={`flex flex-col gap-1.5 transition-all duration-300 ${reqLinkedIn ? "opacity-100 h-auto" : "opacity-0 h-0 overflow-hidden hidden"}`}
                  >
                    <label className="text-xs font-semibold text-slate-600">
                      LinkedIn Profile
                    </label>
