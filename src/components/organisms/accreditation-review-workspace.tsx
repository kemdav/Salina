...
function initialStatuses(orgId: number): Record<string, DocStatus> {
    return Object.fromEntries(ORG_DETAILS[orgId].documents.map((document) => [document.name, document.status]));
}
...
function CheckIcon() {
    return (
        <svg className="h-8 w-8 text-primary" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" aria-hidden="true">
...
function XIcon() {
    return (
        <svg className="h-8 w-8 text-destructive" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" aria-hidden="true">
...
export function AccreditationReviewWorkspace() {
    ...
    const reviewedCount = detail.documents.filter((document) => docStatuses[document.name] !== document.status).length;
    ...
    function saveEdit(id: string) {
        ...
        setNotes((prev) => prev.map((note) => (note.id === id ? { ...note, text: editingText.trim() } : note)));
        setEditingNoteId(null);
        setEditingText('');
    }
    ...
    return (
        <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
            <div>
                <p className="text-sm font-light uppercase tracking-wide text-slate-500">Super Admin Review</p>
                <h1 className="mt-1 text-4xl font-bold leading-none tracking-tight text-foreground" style={{ fontFamily: 'var(--font-heading)' }}>
                    Review Application.
                </h1>
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`block w-full border-b border-border px-4 py-4 text-left transition-colors last:border-0 ${
                                            isSelected ? 'border-l-2 border-l-primary bg-primary/5' : 'hover:bg-slate-50'
                                        }`}
                                    >
                                        <div className="flex items-start justify-between">
                                            <span className="text-[10px] uppercase tracking-widest text-slate-500">{org.priority}</span>
                                            <span className="text-[10px] text-slate-500">{org.time}</span>
                                        </div>
                                        <p className="mt-1 text-sm font-semibold text-foreground">{org.name}</p>
                        ...
                        <div className="flex flex-col gap-6 p-6">
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-slate-500">Submission ID: {detail.submissionId}</p>
                                </div>
                                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-border bg-slate-100">
                                    <span className="text-[10px] italic text-slate-500">logo</span>
                                </div>
                            </div>
                                            ...
                                            <li key={name} className="border-b border-border last:border-0">
                                                ...
                                                >
                                                    ...
                                                    <div className="flex shrink-0 items-center gap-2">
                                                        ...
                                                        >
                                                            {isDownloading ? <span className="text-xs text-slate-500">Downloading…</span> : '↓'}
                                                        </button>
                                                    </div>
                                                </div>
                                                ...
                                                {isExpanded ? (
                                                    <div
                                                        className="mx-1 mb-3 rounded-[var(--radius)] border border-border bg-slate-50/50 p-3"
                                                    ...
                                                    >
                                                        ...
                                                        <textarea
                                                            className="w-full resize-none rounded-[var(--radius)] border border-border bg-background p-2.5 text-sm text-foreground outline-none transition duration-200 focus:border-primary focus:ring-4 focus:ring-primary/10 placeholder:text-muted"
                                                            placeholder="Add notes about this document..."
                                                            rows={2}
                                                            value={docNotes[name] ?? ''}
                                                            onChange={(event) => setDocNotes((prev) => ({ ...prev, [name]: event.target.value }))}
                                                        />
                            ...
                            <div className="border-t border-border pt-4">
                                ...
                                <textarea
                                    className="w-full resize-none rounded-[var(--radius)] border border-border bg-background p-3 text-sm text-foreground outline-none transition duration-200 focus:border-primary focus:ring-4 focus:ring-primary/10 placeholder:text-muted"
                                ...
                                {notes.map((note) => (
                                    <div key={note.id} className="mt-2 rounded-[var(--radius)] border border-border bg-slate-50 p-3">
                                        {editingNoteId === note.id ? (
                                            <>
                                                <textarea
                                                    className="w-full resize-none rounded-[var(--radius)] border border-border bg-background p-2 text-sm text-foreground outline-none transition duration-200 focus:border-primary focus:ring-4 focus:ring-primary/10"
                                                    rows={2}
                                                    value={editingText}
                                                    onChange={(event) => setEditingText(event.target.value)}
                                                />
                                                <div className="mt-2 flex gap-2">
                                                    <button type="button" className="rounded-full border border-primary/30 bg-primary/5 px-2.5 py-1 text-xs font-medium text-primary transition-colors hover:bg-primary/10" onClick={() => saveEdit(note.id)}>
                                                        Save
                                                    </button>
                                                    <button type="button" className="rounded-full border border-border px-2.5 py-1 text-xs font-medium transition-colors hover:bg-slate-100" style={{ color: 'var(--muted)' }} onClick={cancelEdit}>
                                                        Cancel
                                                    </button>
                                                </div>
                                            </>
                                            ...
                                            <div className="flex items-start justify-between gap-2">
                                                <div>
                                                    ...
                                                    <p className="mt-1 text-xs text-slate-500">{note.timestamp} · Super Admin</p>
                                                </div>
                                                <div className="flex shrink-0 gap-1">
                                                    <button type="button" className="rounded px-2 py-0.5 text-xs transition-colors hover:bg-slate-200" style={{ color: 'var(--muted)' }} onClick={() => startEdit(note)}>
                                                        Edit
                                                    </button>
                                                    <button type="button" className="rounded px-2 py-0.5 text-xs text-destructive transition-colors hover:bg-destructive/10" onClick={() => deleteNote(note.id)}>
                                                        Delete
                                                    </button>
                                                </div>
                                            </div>
                            ...
                            <div className="flex items-center justify-between border-t border-border pt-4">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <span title={!canApprove ? 'Review all documents before approving' : undefined}>
                                        <Button disabled={!canApprove} onClick={() => setDecision('approved')}>
                ...
                {decision === 'approved' ? (
                    <div className="flex flex-col items-center justify-center px-6 py-20 text-center">
                        ...
                        <h2 className="mb-2 text-2xl font-bold text-foreground" style={{ fontFamily: 'var(--font-heading)' }}>
                            Accreditation Approved
                        </h2>
                        <p className="max-w-sm text-sm text-slate-500">
                            {detail.name} has been approved. The owner will receive an email invitation to complete their organization setup.
                        </p>
                    </div>
                ) : null}
