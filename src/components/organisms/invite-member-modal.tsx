...
export function InviteMemberModal() {
    ...
    const [emailData, setEmailData] = useState<{email: string, subject: string, body: string} | null>(null);
    ...
    const handleGmail = () => {
        if (!emailData) return;
        window.open(`https://mail.google.com/mail/?view=cm&fs=1&to=${emailData.email}&su=${emailData.subject}&body=${emailData.body}`, '_blank');
        setEmailData(null);
    };

    const handleOutlook = () => {
        if (!emailData) return;
        window.open(`https://outlook.office.com/mail/deeplink/compose?to=${emailData.email}&subject=${emailData.subject}&body=${emailData.body}`, '_blank');
        setEmailData(null);
    };
    ...
    return (
        <>
            <Button onClick={() => { setIsOpen(true); setError(''); }}>+ Invite Member</Button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                        ...
                        <p className="mt-2 text-sm text-slate-500">
                            Invite a new member to join the organization. They will receive an email invitation.
                        </p>
                        ...
                        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
                            ...
                            <div>
                                ...
                                <select
                                    id="role"
                                    name="role"
                                    className="h-10 w-full rounded-(--radius) border border-border bg-white px-3 text-sm text-foreground outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10"
                                >
                            ...
                            <div className="mt-4 flex justify-end gap-3">
                                <Button type="button" variant="secondary" onClick={() => { setIsOpen(false); setError(''); }}>
                                    Cancel
                                </Button>
