...
export function MemberEventsManager({
  ...
  const [attendance, setAttendance] = useState<MemberAttendance[]>(initialAttendance);
  ...
  async function handleParticipate(eventId: string) {
    ...
    try {
      ...
      setAttendance([...attendance, { event_id: eventId, status: "Pending" }]);
    } catch (err: any) {
      alert(err.message);
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-2">
                      <Button variant="secondary" onClick={() => setSelectedEvent(event)}>View Details</Button>
                      {att && (
                        <span className={`inline-flex rounded-full border px-2 py-0.5 text-xs font-medium ${
                          att.status === 'Verified' ? 'bg-success/10 text-success border-success/30' :
                          att.status === 'Pending' ? 'bg-warning/10 text-warning border-warning/30' :
                          att.status === 'Flagged' ? 'bg-destructive/10 text-destructive border-destructive/30' :
                    ...
                    <div key={i} className="min-h-20 border-b border-r border-border p-2 last:border-r-0 nth-[7n]:border-r-0">
                      {day && (
                        <>
                          ...
                          {dayEvents.map(e => (
                            <div key={e.id} onClick={() => setSelectedEvent(e)} className="mb-1 truncate cursor-pointer rounded bg-primary/10 px-1 text-xs text-primary hover:bg-primary/20" title={e.title}>
                              {e.title}
                            </div>
                          ))}
                        </>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-border rounded-xl p-5 shadow-sm">
            <h3 className="font-bold text-foreground mb-4" style={{ fontFamily: "var(--font-heading)" }}>My Participations</h3>
            <div className="space-y-4">
              {attendance.length === 0 && <p className="text-sm text-slate-500">You haven't participated in any events yet.</p>}
              {attendance.map(att => {
                const event = events.find(e => e.id === att.event_id);
                if (!event) return null;
                return (
                  <div key={att.event_id} className="flex items-center justify-between border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                    <div>
                      <p className="text-sm font-medium text-slate-800">{event.title}</p>
                      <p className="text-xs text-slate-500">{new Date(event.start_time).toLocaleDateString()}</p>
                    </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-8 relative shrink-0 bg-primary text-primary-foreground">
              <div className="relative z-10 pr-8">
                <h2 className="text-2xl font-bold leading-tight" style={{ fontFamily: "var(--font-heading)" }}>
                  {selectedEvent.title}
                </h2>
              </div>
              <button
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-800">{new Date(selectedEvent.start_time).toLocaleString()}</p>
                  </div>
                </div>
                <div className="flex gap-3">
                  <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center shrink-0">
              ...
              <div className="pt-4 border-t border-slate-100">
                <h3 className="text-[11px] uppercase tracking-wider font-bold text-slate-400 mb-2">About This Event</h3>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 shrink-0">
              ...
              {attendance.some(a => a.event_id === selectedEvent.id) ? (
                <Button disabled variant="secondary">Already Going</Button>
              ) : (
                <Button onClick={() => handleParticipate(selectedEvent.id)} disabled={isSubmitting}>Participate</Button>
              )}
            </div>
          </div>
        </div>
