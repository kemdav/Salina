  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 sm:p-5 shadow-sm hover:shadow-md transition-all flex flex-col sm:flex-row gap-4 sm:items-center">
      ...
      <div
        ...
        style={{
          backgroundColor: application.branding.primaryColor,
          color: application.branding.textColor,
        }}
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
          <span className="font-medium">
            Stage:{" "}
            <span className="text-slate-700">{application.currentStage}</span>
          </span>
      ...
      <div className="shrink-0 flex sm:flex-col gap-2 mt-2 sm:mt-0 sm:pl-4 sm:border-l border-slate-100">
        <button
          ...
          className="w-full sm:w-auto px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors shadow-sm cursor-pointer"
        >
          View Status
        </button>
        {(application.status === "Pending" ||
          application.status === "In Review") && (
          <button className="w-full sm:w-auto px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors cursor-pointer">
            Withdraw
          </button>
        )}
      </div>
